-- ============================================================
-- RIO ESPORTS
-- Migration 006 - Competitive matchmaking core
-- ============================================================
...
create index idx_matchmaking_queue_player_game_profile
  on public.matchmaking_queue_entries(player_game_profile_id);

alter table public.matches
  add column if not exists queue_mode_id uuid
  references public.queue_modes(id)
  on delete set null;
...
begin
  ...
  where id = new.queue_mode_id;

  select profile_id, is_player
  into v_profile_id, v_is_player
  from public.player_game_profiles
  where id = new.player_game_profile_id;

  if v_game_id is null or v_profile_id is null then
    raise exception 'invalid matchmaking references';
  end if;

  if v_profile_id <> new.profile_id or not v_is_player then
    raise exception 'player game profile is not eligible for matchmaking';
  end if;

  if not exists (
    select 1 from public.player_game_profiles pgp
    where pgp.id = new.player_game_profile_id
      and pgp.game_id = v_game_id
  ) then
...
revoke all on function private.validate_matchmaking_queue_entry() from public, anon, authenticated;
...
create or replace function private.set_matchmaking_queue_updated_at()
returns trigger
language plpgsql
security invoker
set search_path = ''
...
revoke all on function private.set_matchmaking_queue_updated_at() from public, anon, authenticated;

create trigger matchmaking_queue_entries_set_updated_at
before update on public.matchmaking_queue_entries
for each row execute function private.set_matchmaking_queue_updated_at();
...
create policy "Active queue modes are publicly readable"
on public.queue_modes for select
using (is_active = true);

create policy "Players can read their own queue entries"
on public.matchmaking_queue_entries for select
using (profile_id = auth.uid());

-- Queue mutations are intentionally server-owned. No INSERT, UPDATE or
-- DELETE client policies are introduced in this migration.
