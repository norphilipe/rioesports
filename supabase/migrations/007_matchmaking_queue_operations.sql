-- ============================================================
-- RIO ESPORTS
-- Migration 007 - Matchmaking queue operations
-- ============================================================

-- Player-facing queue mutations are exposed as narrow RPCs. Table-level
-- INSERT/UPDATE/DELETE remains unavailable to authenticated clients.
...
begin
  ...
  select profile_id, game_id, mmr
    into v_profile_id, v_game_id, v_mmr
  ...
  -- Keep the latest intent idempotent: calling join twice for the same active
  -- queue returns the existing entry instead of creating duplicates.
  select * into v_entry
  ...
  -- A player may only have one active matchmaking intent at a time.
  if exists (
    select 1
    from public.matchmaking_queue_entries
...
-- Match formation is intentionally private. It is not executable by anon or
-- authenticated clients and must be called by a trusted server worker.
...
begin
  select team_size, game_id
    into v_team_size, v_game_id
  from public.queue_modes
  where id = p_queue_mode_id
    and is_active = true
  for update;
  ...
  select array_agg(id order by queued_at, id)
    into v_entry_ids
  from (
    select id
    from public.matchmaking_queue_entries
