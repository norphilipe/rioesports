-- RIO ESPORTS
-- Migration 014 - Harden competitive result processing

create unique index if not exists rating_history_one_result_per_match_player
  on public.rating_history(player_game_profile_id, match_id)
...
begin
  ...
  loop
    if v_player.won is null then
      raise exception 'all players must have a confirmed result';
    end if;
