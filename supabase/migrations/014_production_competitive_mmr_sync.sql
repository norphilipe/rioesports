-- RIO ESPORTS
-- Migration 014 - Production competitive MMR synchronization
-- Adds the atomic rating function required by the production result flow.
...
begin
  ...
  for v_player in select profile_id, won from public.match_players where match_id = p_match_id loop
    if v_player.won is null then
      raise exception 'all match players require a confirmed result';
    end if;
    ...
    if exists (
      ...
      where player_game_profile_id = v_game_profile.id and match_id = p_match_id
    ) then
      raise exception 'rating already applied for player and match';
    end if;
    ...
    set mmr = v_next,
        ...
        provisional = case when placement_matches + 1 >= 10 then false else provisional end,
        updated_at = now()
...
grant execute on function public.apply_competitive_match_rating(uuid) to authenticated;
