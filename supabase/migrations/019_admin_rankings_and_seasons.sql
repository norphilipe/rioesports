...
create unique index if not exists game_seasons_one_active_per_game
  on public.game_seasons(game_id)
  where is_active;

create index if not exists player_game_profiles_ranking_lookup
  on public.player_game_profiles(game_id, mmr desc)
  where is_player = true;

create or replace function public.admin_create_game_season(
  target_game_id uuid,
  season_name text,
  starts_at_value timestamptz,
  ends_at_value timestamptz default null,
  activate_now boolean default false
)
returns uuid
language plpgsql
security definer
...
begin
  ...
  if ends_at_value is not null and ends_at_value <= starts_at_value then
    raise exception 'season end must be after its start';
  end if;

  if activate_now then
    update public.game_seasons
      set is_active = false
      where game_id = target_game_id and is_active;
  end if;

  insert into public.game_seasons (game_id, name, starts_at, ends_at, is_active)
  values (target_game_id, trim(season_name), starts_at_value, ends_at_value, activate_now)
...
create or replace function public.admin_set_game_season_active(
...
create or replace function public.admin_ranking_snapshot(
  target_game_id uuid default null,
  result_limit integer default 100
)
returns table(
  player_game_profile_id uuid,
  profile_id uuid,
  username text,
  display_name text,
  game_id uuid,
...
as $$
  ...
  join public.profiles p on p.id = pgp.profile_id
  join public.games g on g.id = pgp.game_id
  where public.is_current_platform_admin()
    and pgp.is_player = true
    and (target_game_id is null or pgp.game_id = target_game_id)
