-- ============================================================
-- RIO ESPORTS
-- Migration 010 - Matchmaking queue validation
-- ============================================================
...
begin
  ...
  select profile_id, is_player into v_profile_id, v_is_player
  ...
  if not exists (
    select 1
    from public.player_game_profiles
    where id = new.player_game_profile_id and game_id = v_game_id
  ) then
...
for each row
execute function private.validate_matchmaking_queue_entry();
