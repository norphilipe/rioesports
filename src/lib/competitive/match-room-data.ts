...
export async function getMatchRoomData(matchId: string): Promise<MatchRoomData | null> {
  ...
  const { data: teams } = await supabase.from("match_teams").select("id,side").eq("match_id", matchId);
  const teamAId = teams?.find((team) => team.side === "team_a")?.id ?? null;
  const teamBId = teams?.find((team) => team.side === "team_b")?.id ?? null;
  ...
  return { state: statusMap[match.status] ?? "awaiting_result", teamA, teamB, teamAId, teamBId };
}
