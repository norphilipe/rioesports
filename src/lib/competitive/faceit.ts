...
function getFaceitApiKey() {
  ...
  if (!apiKey) {
    throw new Error("FACEIT_DATA_API_KEY is not configured.");
  }
  return apiKey;
}
...
async function requestFaceitPlayer(game: string, steamId64: string) {
  ...
  if (!response.ok) {
    throw new Error(`FACEIT player lookup failed with status ${response.status}.`);
  }
...
export async function getFaceitPlayerBySteamId(steamId64: string) {
  // FACEIT's CS2 identifiers have evolved from csgo to cs2. Try the current
  // identifier first and retain the legacy identifier for existing accounts.
