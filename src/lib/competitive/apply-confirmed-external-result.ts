...
export async function applyConfirmedExternalResult(matchId: string) {
  ...
  const { data: submissions, error: submissionsError } = await supabase
    ...
    .select("winner_team_id,loser_team_id,winner_score,loser_score")
    .eq("match_id", matchId);

  if (submissionsError) throw new Error(submissionsError.message);

  const first = submissions?.[0];
  const confirmed = first && (submissions ?? []).filter((item) =>
    item.winner_team_id === first.winner_team_id &&
    item.loser_team_id === first.loser_team_id &&
    item.winner_score === first.winner_score &&
    item.loser_score === first.loser_score,
  ).length >= 2;
  ...
  const { error: winnerError } = await supabase
    .from("match_teams")
    .update({ score: first.winner_score, won: true })
    .eq("id", winner.id);
  if (winnerError) throw new Error(winnerError.message);

  const { error: loserError } = await supabase
    .from("match_teams")
    .update({ score: first.loser_score, won: false })
    .eq("id", loser.id);
  ...
  const { error: playersError } = await supabase
    ...
    .update({ won: true })
    .eq("match_id", matchId)
    .eq("team_side", winner.side);
  if (playersError) throw new Error(playersError.message);

  const { error: losingPlayersError } = await supabase
    .from("match_players")
    .update({ won: false })
    .eq("match_id", matchId)
    .eq("team_side", loser.side);
