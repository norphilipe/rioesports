...
export default async function AdminPage() {
  ...
  const [tournamentsResult, teamsResult, queueResult, usersResult, newsResult, moderationResult] = await Promise.all([
    supabase.from("tournaments").select("id", { count: "exact", head: true }),
    supabase.from("teams").select("id", { count: "exact", head: true }),
    supabase.from("matchmaking_queue_entries").select("id", { count: "exact", head: true }).eq("status", "queued"),
    supabase.from("profiles").select("id", { count: "exact", head: true }),
    supabase.from("platform_news").select("id", { count: "exact", head: true }).not("published_at", "is", null).lte("published_at", now),
    supabase.from("platform_moderation_actions").select("id", { count: "exact", head: true }).or(`expires_at.is.null,expires_at.gt.${now}`),
  ]);
