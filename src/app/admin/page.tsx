...
export default async function AdminPage() {
  ...
  if (!isAdmin) redirect("/");
  const [tournamentsResult, teamsResult, queueResult, rankingResult] = await Promise.all([
    supabase.from("tournaments").select("id", { count: "exact", head: true }),
    supabase.from("teams").select("id", { count: "exact", head: true }),
    supabase.from("matchmaking_queue_entries").select("id", { count: "exact", head: true }).eq("status", "queued"),
    supabase.from("player_game_profiles").select("id", { count: "exact", head: true }),
  ]);
  const cards = [
    ...
    { label: "Jogadores ranqueados", value: rankingResult.count ?? 0, href: "/admin/rankings", description: "Acompanhar ranking e temporadas" },
