...
export default async function AdminPage() {
  ...
  if (!isAdmin) redirect("/");
  const [tournamentsResult, teamsResult, queueResult, usersResult, newsResult, penaltiesResult] = await Promise.all([
    ...
    supabase.from("user_penalties").select("id", { count: "exact", head: true }).is("revoked_at", null),
  ]);
  const cards = [
  ...
  const modules = [
    ...
    ["🛡️", `Moderação (${penaltiesResult.count ?? 0})`, "/admin/moderation", "Usuários e penalidades"],
  ] as const;
