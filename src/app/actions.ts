...
export async function updateProfileAction(formData: FormData): Promise<void> {
  ...
  const fail = (message: string): never => redirect(`/perfil?profile=error&message=${encodeURIComponent(message)}`);

  if (!validateUsername(username).ok) fail("Nome de usuário inválido.");
  if (displayName.length < 2 || displayName.length > 60) fail("O nome de exibição deve ter entre 2 e 60 caracteres.");
  ...
  if (normalizedCity !== "N/A" && !validCities.has(normalizedCity)) fail("Selecione um município válido do estado do Rio de Janeiro ou N/A.");
